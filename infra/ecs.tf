  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        {
          name  = "DATABASE_URL"
          value = "postgresql://${module.db.db_instance_username}:${var.db_password}@${module.db.db_instance_endpoint}/collm_core?sslmode=no-verify"
        },
        {
          name  = "AWS_REGION"
          value = local.region
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        {
          name  = "DATABASE_URL"
          value = "postgresql://${module.db.db_instance_username}:${var.db_password}@${module.db.db_instance_endpoint}/collm_users?sslmode=no-verify"
        }
      ]
      logConfiguration = {
        logDriver = "awslogs"
